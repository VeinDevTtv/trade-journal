...
router.get('/',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const bestDayResult = await executeQuery<{
        date: string;
        profit: number;
        trades: number;
      }>(`
      ...
      const profitBySymbol = await executeQuery<{
        symbol: string;
        profit: number;
        trades: number;
      }>(`
      ...
      const profitByDay = await executeQuery<{
        day: string;
        profit: number;
        trades: number;
      }>(`
      ...
      const equityCurveData = await executeQuery<{
        date: string;
        daily_profit: number;
      }>(`
        SELECT 
...
router.get('/performance-by-symbol',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const symbolPerformance = await executeQuery<{
        symbol: string;
        total_trades: number;
        winning_trades: number;
        losing_trades: number;
...
router.get('/risk-metrics',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const riskMetrics = await executeQuery<{
        average_rrr: number;
        max_drawdown: number;
        largest_win: number;
        largest_loss: number;
      ...
      const equityData = await executeQuery<{
        date: string;
