...
async function seedDatabase() {
    ...
    try {
        ...
        const userResult = await (0, database_1.executeSingle)(`INSERT INTO users (discord_id, username, discriminator, avatar, email) 
       VALUES (?, ?, ?, ?, ?)
       ...
       email = VALUES(email)`, [
        ...
        for (const account of sampleAccounts) {
            const accountResult = await (0, database_1.executeSingle)(`INSERT INTO accounts (user_id, name, type, balance, currency) 
         VALUES (?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE 
         balance = VALUES(balance),
         type = VALUES(type)`, [userId, account.name, account.type, account.balance, account.currency]);
        ...
        for (let i = 0; i < sampleTrades.length; i++) {
            ...
            // Insert trade
            const tradeResult = await (0, database_1.executeSingle)(`INSERT INTO trades (
          ...
          is_win, trade_date, trade_time, notes
            ...
            for (const tag of trade.tags) {
                await (0, database_1.executeSingle)('INSERT INTO trade_tags (trade_id, tag) VALUES (?, ?)', [tradeId, tag]);
            }
        }
        // Create user settings
        console.log('Creating sample user settings...');
        await (0, database_1.executeSingle)(`INSERT INTO user_settings (
